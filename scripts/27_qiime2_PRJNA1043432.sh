#!/bin/bash
# PRJNA1043432 QIIME2 流程：双端 2×250bp，V3-V4，341F/806R，拭子；ITS 样本由 cutadapt 自动过滤
# 数据类型：双端 paired-end，引物未截除；数据集含 ITS 真菌测序样本
#
# 用法：
#   conda activate qiime2-amplicon-2023
#   cd /home/usr/yel/exp-OLP_meta/pipeline
#   nohup bash scripts/27_qiime2_PRJNA1043432.sh > logs/qiime2_PRJNA1043432.log 2>&1 &
...
# Step 3: cutadapt
# 数据集含 ITS 真菌测序样本，cutadapt --discard-untrimmed 会自动排除（ITS 样本无 16S 引物）
echo "[$(date '+%H:%M:%S')] cutadapt 截除引物（ITS 样本将被自动丢弃）..."
...
# Step 4: DADA2
# V3-V4 扩增子去引物后 ~428bp，250bp reads 重叠最小（240+200-428=12bp）
